...
Format/structure is copied but all cell values are cleared (template-only transfer).
"""
...
def transfer_sheet(src_ws, dst_ws):
    # 1. Clear destination sheet content entirely (skip read-only MergedCell slaves)
    for row in dst_ws.iter_rows():
        for cell in row:
            if cell.__class__.__name__ != "MergedCell":
                cell.value = None

    # 2. Remove existing merged cells in destination
    ...
    # 4. Copy column dimensions (preserves column count/width, not changed)
    ...
    # 5. Copy cell styles only (no values — template mode)
    for row in src_ws.iter_rows():
        for src_cell in row:
            dst_cell = dst_ws.cell(row=src_cell.row, column=src_cell.column)
            copy_cell_style(src_cell, dst_cell)
            # value intentionally left as None

    # 6. Replicate merged cells
    for merge in src_ws.merged_cells.ranges:
        dst_ws.merge_cells(str(merge))

    # 7. Copy sheet-level print/page settings
...
def main():
    ...
    for src_name, dst_name in SHEET_MAP:
        ...
        print(f"    src size: {src_ws.max_row}r × {src_ws.max_column}c  |  merged: {len(list(src_ws.merged_cells.ranges))}")
        print(f"    dst size after transfer: {dst_ws.max_row}r × {dst_ws.max_column}c  |  merged: {len(list(dst_ws.merged_cells.ranges))}")
